#!/usr/bin/env bash
# Restore the Linux/WSL or macOS Herdr setup without requiring Stow.
set -euo pipefail
...
case $(uname -s) in
    Linux)
        auto_title_config_dir=$config_dir/herdr-auto-title
        data_dir=${XDG_DATA_HOME:-$HOME/.local/share}
        ;;
    Darwin)
        # Auto Title uses Go's os.UserConfigDir, which ignores XDG on macOS.
        auto_title_config_dir="$HOME/Library/Application Support/herdr-auto-title"
        data_dir=${XDG_DATA_HOME:-$HOME/Library/Application Support}
        ;;
    *) echo 'This setup supports Linux/WSL and macOS.' >&2; exit 1 ;;
esac
[[ -z ${HERDR_CONFIG_PATH:-} ]] || { echo 'Unset HERDR_CONFIG_PATH before setup.' >&2; exit 1; }
for dependency in herdr git go yazi python3; do
...
copy_config "$repo_dir/herdr/.config/herdr-auto-title/config.env" "$auto_title_config_dir/config.env"
